...
#  Choices: any of [0, 10, 20, 30, 40, 50, 'DEBUG', 'INFO', 'WARN', 'ERROR', 'CR                                                                                                                                                             ITICAL']
#  Default: 30
...
#  servers until a server is shutdown. Spawn requests will be rejected with a 42                                                                                                                                                             9
...
#
#          Allows ahead-of-time generation of API tokens for use by externally m                                                                                                                                                             anaged services,
...
#
#          If both this and cleanup_proxy are False, sending SIGINT to the Hub w                                                                                                                                                             ill
#          only shutdown the Hub, leaving everything else running.
#
#          The Hub should be able to resume from database state.
#  Default: True
...
#  the Hub or the underlying spawning system. Set this limit to prevent bursts o                                                                                                                                                             f
...
#  be rejected with a 429 error asking them to try again. Users will have to wai                                                                                                                                                             t
...
## If named servers are enabled, default name of server to spawn or open, e.g. b                                                                                                                                                             y
#  user-redirect.
#  Default: ''
...
#
#          Use when the bind address (`hub_ip`) is 0.0.0.0, :: or otherwise diff                                                                                                                                                             erent
#          from the connect address.
#
#          Default: when `hub_ip` is 0.0.0.0 or ::, use `socket.gethostname()`,
#  otherwise use `hub_ip`.
#
#          Note: Some spawners or proxy implementations might not support hostna                                                                                                                                                             mes. Check your
...
#
#          By default, the hub listens on localhost only. This address must be a                                                                                                                                                             ccessible from
#          the proxy and user servers. You may need to set this to a public ip o                                                                                                                                                             r '' for all
#          interfaces if the proxy or user servers are in containers or on a dif                                                                                                                                                             ferent host.
#
#          See `hub_connect_ip` for cases where the bind and connect address sho                                                                                                                                                             uld differ,
...
#
#          This is the internal port of the hub itself. It should never be acces                                                                                                                                                             sed directly.
#          See JupyterHub.port for the public port to use when accessing jupyter                                                                                                                                                             hub.
#          It is rare that this port should be set except in cases of port confl                                                                                                                                                             ict.
#
...
#          listen on all interfaces. This is the only address through which Jupy                                                                                                                                                             terHub
#          should be accessed by users.
#
#          .. deprecated: 0.9
#              Use JupyterHub.bind_url
#  Default: ''
c.JupyterHub.ip = 'wvm-srv5.luminy.univ-amu.fr'
...
#
#          Loading one set of groups, then starting JupyterHub again with a diff                                                                                                                                                             erent
...
## DEPRECATED since version 0.8: Use ConfigurableHTTPProxy.check_running_interva                                                                                                                                                             l
#  Default: 5
...
#          if JupyterHub is being run behind a reverse proxy or services using s                                                                                                                                                             sl
...
#          the `X-Forwarded-For` header. For example, if an external proxy is us                                                                                                                                                             ed for TLS
#          termination, its IP address should be added to this list to ensure th                                                                                                                                                             e correct
#          client IP addresses are recorded in the logs instead of the proxy ser                                                                                                                                                             ver's IP
#          address.
#  Default: []
...
#      - poll
#
#      As JupyterHub supports multiple users, an instance of the Spawner subclas                                                                                                                                                             s
...
#
#  This is usually set if you want to start the single-user server in a differen                                                                                                                                                             t
...
#
#  **This is a configuration setting. Your spawner must implement support for th                                                                                                                                                             e
#  limit to work.** The default spawner, `LocalProcessSpawner`, does **not**
#  implement this support. A custom spawner **must** add support for this settin                                                                                                                                                             g
...
#  Example uses:
#
#  - You can set `notebook_dir` to `/` and `default_url` to `/tree/home/{usernam                                                                                                                                                             e}` to allow people to
#    navigate the whole filesystem from their notebook server, but still start i                                                                                                                                                             n their home directory.
#  - Start with `/notebooks` instead of `/tree` if `default_url` points to a not                                                                                                                                                             ebook instead of a directory.
...
#  environment, such as when they have their own conda environments / virtualenv                                                                                                                                                             s
#  / containers.
#  Default: False
...
#  Default: ['PATH', 'PYTHONPATH', 'CONDA_ROOT', 'CONDA_DEFAULT_ENV', 'VIRTUAL_E                                                                                                                                                             NV', 'LANG', 'LC_ALL', 'JUPYTERHUB_SINGLEUSER_APP']
...
#
#  Environment variables that end up in the single-user server's process come fr                                                                                                                                                             om 3 sources:
#    - This `environment` configurable
#    - The JupyterHub process' environment variables that are listed in `env_kee                                                                                                                                                             p`
#    - Variables to establish contact between the single-user notebook and the h                                                                                                                                                             ub (such as JUPYTERHUB_API_TOKEN)
#
#  The `environment` configurable should be set by JupyterHub administrators to
#  add installation specific environment variables. It is a dict where the key i                                                                                                                                                             s
...
#
#  `~` will be expanded to the home directory of the user, and {username} will b                                                                                                                                                             e
...
## An optional hook function that you can implement to do some bootstrapping wor                                                                                                                                                             k
...
#  takes longer than this. start should return when the server process is starte                                                                                                                                                             d
...
#  tokens.
#
#          See :meth:`.refresh_user` for what happens when user auth info is ref                                                                                                                                                             reshed
#          (nothing by default).
#  Default: 300
...
#  This is an additional block list that further restricts users, beyond whateve                                                                                                                                                             r
#  restrictions the authenticator has in place.
#
#  If empty, does not perform any additional restriction.
#
...
#          any users that do not pass validation will be deleted from the databa                                                                                                                                                             se.
...
#          spawn_data = {
#              'pw_data': user_data
#              'gid_list': os.getgrouplist(authentication['name'], user_data.pw_                                                                                                                                                             gid)
#          }
#
